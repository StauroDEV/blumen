...
import { SafeContract } from '../types.js'
...
class OwnerManager {
  ...
  constructor(publicClient: PublicClient, safeContract?: SafeContract) {
    this.#publicClient = publicClient
    this.#safeContract = safeContract
  }

  private validateOwnerAddress(ownerAddress: string, errorMessage?: string): void {
    const isValidAddress = isAddress(ownerAddress)
  ...
  async encodeAddOwnerWithThresholdData(ownerAddress: string, threshold?: bigint): Promise<string> {
    if (!this.#safeContract) {
      throw new Error('Safe is not deployed')
    }
