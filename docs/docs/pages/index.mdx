...
- **Multi-Provider**: Replicate the website to various content storage providers in a single command.
...
- **Secured by Safe**: Extra factor of security with [Safe](https://safe.global).
- **Tiny size**: Less than 1MB single file distribution that is easy to audit.
...
Blumen integrates with CI/CD pipelines for automatic dApp deployment.
