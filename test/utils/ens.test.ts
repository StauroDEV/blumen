...
describe('ens utils', () => {
  describe('prepareUpdateEnsArgs', () => {
    it('should properly encode a valid IPFS CIDv1', () => {
      ...
      assert.equal(contentHash, 'e30101701220652e92f7b05443244a6b8c3db1dfe2e820506b8485b1e51794580cb05a8ab599')
      assert.equal(node, '0x6dd56164f699a101d6063add452dfed7c6c09fe17b8e4acf3328f9387f5030b9')
    })
    it('should properly encode a valid Swarm Reference', () => {
      ...
      assert.equal(contentHash, 'e40101fa011b20d1de9994b4d039f6548d191eb26786769f580809256b4685ef316805265ea162')
      assert.equal(node, '0x6dd56164f699a101d6063add452dfed7c6c09fe17b8e4acf3328f9387f5030b9')
    })
  ...
  describe('chainToRpcUrl', () => {
    it('returns default RPC URLs for mainnet and sepolia', () => {
      assert.equal(chainToRpcUrl('mainnet'), 'https://ethereum-rpc.publicnode.com')
      assert.equal(chainToRpcUrl('sepolia'), 'https://ethereum-sepolia-rpc.publicnode.com')
    })
  })
})
